...
def resTar(tar):
    ...
    for tarea in tar:
        ...
        if tarea.entregado:
            ...
            h='Sin hacer'
        res += tarea.nombre + '\n' + tarea.descripcion + '\n' + f'Nota: {str(tarea.nota)}' + '\n' + h + '\n' + f'Fecha: {str(tarea.fecha)}' + '\n'
    return res
...
def create(request):
    ...
    if request.method == 'GET':
        ...
        conversation = [
            ...
            "Nro de faltas", f"Hola {nombre}, tienes {faltas} faltas. Ten cuidado!",
            "Nro de Faltas", f"Hola {nombre}, tienes {faltas} faltas. Ten cuidado!",
            "Tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "Mis tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "mis Tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "Mis Tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "mis tareas", f"Hola {nombre}, A continuación te enviaré la información de las tareas: {resTar(tareas)}",
            "tareas no entregadas", f"Hola {nombre}, estas son las tareas que no entregaste: {resTar(tareaN)}",
            "Tareas no entregadas", f"Hola {nombre}, estas son las tareas que no entregaste: {resTar(tareaN)}",
            "Tareas no Entregadas", f"Hola {nombre}, estas son las tareas que no entregaste: {resTar(tareaN)}",
            "tareas no Entregadas", f"Hola {nombre}, estas son las tareas que no entregaste: {resTar(tareaN)}"
        ]
